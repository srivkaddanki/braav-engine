...
def execute_db(query, params=()):
    ...
    print(f"DEBUG [braavcore]: execute_db called. Query: {query}")
...
def librarian_architect(user_input):
    ...
    # Debugging: Trace caller to identify frontend
    stack = inspect.stack()
    caller = stack[1].filename if len(stack) > 1 else "Unknown"
    print(f"DEBUG [braavcore]: librarian_architect called from: {caller}")
    ...
    # 2026 SDK CALL
    print("DEBUG [braavcore]: Calling Gemini...")
    response = client.models.generate_content(
        model="gemini-1.5-flash-8b",
        contents=prompt
    )
    print(f"DEBUG [braavcore]: Gemini response received: {response.text[:50]}...")
